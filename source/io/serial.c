...
static void serial_configure_baud_rate(unsigned short com, unsigned short divisor) // the function that will select the speed used for sending data
{
...
{
    /* RTS = 1 and DTS = 1 */
    outb(SERIAL_MODEM_COMMAND_PORT(com), 0x03); // Book's standard
}

static int serial_is_transmit_fifo_empty(unsigned int com) // Function responsible for checking if the buffer is empty
{
...
{
    while (!serial_is_transmit_fifo_empty(SERIAL_COM1_BASE)); // Stays on the loop while the buffer isn't empty
    outb(SERIAL_DATA_PORT(SERIAL_COM1_BASE), c);
}

int serial_write(char *buf) // Function to write with the serial port
{
